...
This project will track the "modernization" work of the DOGE crew as they steamroll across various agencies. This work will be using only reported source and court filings, which should always be linked from the records where they are cited. It has already presented a more comprehensive picture of events, that will continue to get clearer as more things come to light. You might begin to notice that none of their work seems to involve actual IT modernization...

This data will **not** track contracts being canceled by DOGE, except if that has ramifications for DOGE actions and activities. It's not that it's not relevant to the damage DOGE is doing, I just am finding it harder to keep track off and quantify. It also will **not** track the various unjust layoffs and firings, except where it is relevant to DOGE doing its dirty work. It's not that those events are not important, but I have limited bandwidth as it is. Maybe in the future, I will add them back.

## The Data

Currently, the source data is stored in several files located within the `raw_data` directory:

- `people.yaml`: a list of people associated with DOGE and information about their positions at various agencies
...
If you are using Ruby, like this project is, then you might need to manually parse the dates before using them with the `edtf` gem. Note that I will be an exact date string in the `sort_date` field if you want to just use that, but be wary of false precision ("the middle of March" would be represented as "2025-03-15" possibly)
