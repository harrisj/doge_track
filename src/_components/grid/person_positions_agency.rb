...
module Grid
  ...
  class PersonPositionsAgency < Bridgetown::Component
    include ExtraMixins
           ...
           <details class="collapse">
             <summary class="collapse-title p-0">
              ...
              #{ html_map(@positions) do |pos|
                   <<~HTML
                     <div>#{render Atoms::PositionMoveLabel.new(position: pos)} #{html -> { position_date_range(pos) }} #{render Molecules::PositionSummary.new(position: pos, agency: @agency)}</div>
                   HTML
                 end
              }
              #{html -> { exit_label }}
